jQuery(document).ready(function($) {
    ...
    $("#revelcf-submit").click(function(e) {
        "use strict";
        e.preventDefault();
        var $button=$(this);
        var $form = $button.parents('form');
        var $wrapper = $form.parents('#revelcf-wrapper');
        $wrapper.find('.revelcf-response-output').slideUp(300);
        ...
        $.ajax({
            ...
            success: function(msg){
                if(msg == "OK"){
                    ...
                    $wrapper.find('.revelcf-response-output').addClass('success').html(success_msg).slideDown(600);
                }
                else{
                    $button.val('Submit').prop('disabled', false).removeClass('disabled');
                    $wrapper.find('.revelcf-response-output').html(msg).slideDown(600);
                }
            }
        });
        return false;
